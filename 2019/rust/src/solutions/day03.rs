...
fn part1(input: &(Pair<Vec<Pair<isize>>>, Vec<Pair<isize>>)) -> usize {
    input.1.iter().map(|pt| manhattan_distance(pt)).min().unwrap()
}

fn path_index<'a,'b>(path: &'a Vec<Pair<isize>>, inters: &'b Vec<Pair<isize>>) -> Vec<(usize, &'a Pair<isize>)> {
    let mut path_index:Vec<_> = path.iter().enumerate()
...
fn part2(input: &(Pair<Vec<Pair<isize>>>, Vec<Pair<isize>>)) -> usize {
    ...
    path1_index.iter()
        ...
        .min_by(|a,b| a.0.cmp(&b.0)).unwrap().0
}
